...
export const CARD_KINDS = {
  visa: { label: 'Visa', last4Style: 'V', tint: ['#1A1F71', '#2A337A'] as [string, string], chip: '#E7C97A' },
  mc: { label: 'Mastercard', last4Style: 'M', tint: ['#211E1F', '#3A2A2A'] as [string, string], chip: '#E7C97A' },
  amex: { label: 'Amex', last4Style: 'A', tint: ['#0B7A75', '#0F8E89'] as [string, string], chip: '#E7C97A' },
  unionp: { label: 'UnionPay', last4Style: 'U', tint: ['#1B1F3A', '#2A2F5A'] as [string, string], chip: '#E7C97A' },
};

export const CATEGORIES = [
  { id: 'streaming' as const, label: 'Streaming', swatch: 'oklch(0.72 0.16 25)' },
  { id: 'productivity' as const, label: 'Productivity', swatch: 'oklch(0.82 0.16 110)' },
  { id: 'cloud' as const, label: 'Cloud & Storage', swatch: 'oklch(0.70 0.12 220)' },
  { id: 'school' as const, label: 'School Fees', swatch: 'oklch(0.62 0.18 295)' },
  { id: 'ecommerce' as const, label: 'E-commerce', swatch: 'oklch(0.78 0.14 155)' },
  { id: 'bills' as const, label: 'Bills', swatch: 'oklch(0.68 0.04 220)' },
];
...
function makeSubs(): Subscription[] {
  ...
  type BaseSub = Omit<Subscription, 'history' | 'nextCharge' | 'status'> & {
  ...
  const base: BaseSub[] = [
    {
      ...
      evidence: [
        'Price up 20% — Rs 1,499 → 1,800 effective Jul',
        'Used 22 of last 30 days',
        'Shared with 4 profiles',
      ],
    },
    {
      id: 'spotify',
    ...
    },
    {
      ...
      card: 'visa',
      last4: '4421',
      nextCharge: addDays(now, 18),
      since: '2023-01',
      usage: { sessionsLast30: 6, lastUsed: '6d ago' },
      verdict: 'review',
      evidence: [
        'Used 6 of last 30 days',
        'Overlaps with Spotify (music)',
        'Renewed monthly for 17 months',
      ],
    },
    {
      id: 'disney',
      ...
      evidence: [
        'Used 2 of last 30 days',
        'No new content viewed in 6 weeks',
        'Yearly renewal due Sep 29',
      ],
    },
    {
      id: 'notion',
      ...
      priceIncrease: {
        ...
        toUSD: 10.0,
        date: '2026-04-15',
        emailDate: '2026-04-15',
      },
      evidence: [
        'Price up 25% — Rs 2,225 → 2,780 since Apr',
        'Used 24 of last 30 days',
        '3 workspaces active',
      ],
    },
    {
      ...
      priceIncrease: {
        ...
        toUSD: 20.0,
        date: '2026-05-28',
        emailDate: '2026-05-28',
      },
      evidence: [
        'Price up 25% — Rs 4,450 → 5,560 effective Jun',
        'Used 19 of last 30 days',
        'API not in use — Plus only',
      ],
    },
    {
      id: 'claude',
    ...
    },
    {
      ...
      card: 'amex',
      last4: '1005',
      nextCharge: addDays(now, 47),
      since: '2020-02',
      usage: { sessionsLast30: 3, lastUsed: '11d ago' },
      verdict: 'review',
      evidence: [
        'Used 3 of last 30 days',
        'Only Photoshop opened — single-app plan PKR 89k/yr available',
      ],
    },
    {
      id: 'icloud',
      merchant: 'iCloud+ 2 TB',
      glyph: 'i',
      glyphBg: '#0B0F0E',
      category: 'cloud',
      cycle: 'monthly',
    ...
    },
    {
      ...
      card: 'amex',
      last4: '1005',
      nextCharge: addDays(now, 78),
      since: '2019-11',
      usage: { sessionsLast30: 1, lastUsed: '22d ago' },
      verdict: 'cancel',
      evidence: [
        'Used 1 of last 30 days',
        'Overlaps with iCloud and Google One',
        'No new files in 3 weeks',
      ],
    },
    {
      id: 'lgs',
      merchant: 'LGS Defence — Tuition',
      glyph: 'L',
      glyphBg: '#0E5E2A',
      category: 'school',
      cycle: 'monthly',
  ...
  return base.map((s) => {
    ...
    const monthsBack = s.cycle === 'yearly' ? 3 : 12;
    ...
    for (let i = 1; i <= monthsBack; i++) {
      ...
      let pkr = s.amountPKR;
      let usd = s.amountUSD;
      if (bumpDate && d < bumpDate && s.priceIncrease) {
    ...
    return {
      ...s,
      history,
      nextCharge: fmtISO(s.nextCharge),
      status: s.status ?? 'active',
    };
  });
}

export const SUBS_HEAVY = makeSubs();
const LIGHT_IDS = new Set(['netflix', 'spotify', 'chatgpt', 'icloud', 'lgs', 'beacon', 'prime']);
export const SUBS_LIGHT = SUBS_HEAVY.filter((s) => LIGHT_IDS.has(s.id));

export function buildTrend(subs: Subscription[]) {
  ...
  return months.map((month, i) => {
    ...
    return { month, pkr: Math.round(base * (1 + noise)) };
  });
}

export function toCurrency(sub: Subscription, currency: 'PKR' | 'USD') {
